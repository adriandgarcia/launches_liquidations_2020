---
title: "Shareclass Activity 2020"
knit: (function(input_file, encoding) {
  out_dir <- 'docs';
  rmarkdown::render(input_file,
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'index.html'))})
author: "Adrian D. Garcia"
date: "January 2021"
output:
  html_document:
    toc: true
    toc_float: true
---

# Intro

## Getting Started

Data pulled from Morningstar Direct on Jan. 22, 2020. Files combined using _combine_files.r_. 
Data in M* filtered:

* != Fund of funds: Yes
* != Morningstar Category: Money Market - Non-40 ACT
* != Morningstar Category: Money Market - Taxable
* != Morningstar Category: Money Market - Taxfree

56,284 rows, 36 columns

Added column _Obsolete_ to indicate if share class is in existence or obsolete. I assume a blank Obsolete..Date means the share class still exists.

Also added _Inception_Year_ and _Obsolete_Year_ because these variables are used below.

Data filtered to remove:

* "Load Waived" Share.Class.Type
* Share classes launched and liquidated during the same calendar year
* Share classes launched or liquidated after 2020

48,520 rows, 41 columns

```{r setup, message = FALSE, warning = FALSE}

library(tidyverse) #For Data Analysis
library(lubridate) #For working with dates
library(DT) #For visualizing tables


Full <- read_csv("data_combined.csv",
                 guess_max = 20000) %>% 
  rename_all(make.names) %>% 
  mutate(
    Inception_Year = year(Inception..Date),
    Obsolete_Year = year(Obsolete..Date),
    Obsolete =
           case_when(
             !is.na(Obsolete..Date) ~ "Obsolete",
             TRUE ~ "Exists"
           )) %>% 
  filter(Share.Class.Type != "Load Waived" |
           is.na(Share.Class.Type)) %>% 
  filter((Inception_Year != Obsolete_Year) |
             is.na(Obsolete_Year) |
             is.na(Inception_Year)) %>% 
  filter(Inception_Year != 2021 |
           Obsolete_Year != 2021)

```


# Overview

```{r graph_overview, echo=FALSE}

ggplot(data = Full, mapping = aes(x = year(Inception..Date), fill = Obsolete)) +
  geom_bar(position = "stack") +
  ggtitle("Share Class Launches") +
  ylab("Count") +
  xlab("YEAR") +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))

```

## Net Count

There was a large drop off in launches in 2020 compared to recent years. However it appears rationalization continued at roughly the same levels as recent years.

Of 56,720 fund share classes examined there are inception dates missing for 84.

```{r data_overview, echo = FALSE, message = FALSE}

#A function to count the number of share classes either created or liquidated each year

Year_Count <- function(colz){
   Full %>%
    group_by_at(colz) %>% 
    summarise(Count = n(), .groups = "drop") %>% 
    arrange(desc(.[[1]]))
    }

inception_year <- Year_Count(colz = c("Inception_Year")) #Tallys share classes by year created
obsolete_year <- Year_Count(colz = c("Obsolete_Year")) #Tallys share classes by year liquidated

merge_type <- Year_Count(colz = c("Obsolete_Year", "Obsolete..Type")) #Tallys share classes by year liquidated and liquidation type


#Counts the net number of share classes created/cut

Net_Count <- full_join(inception_year, obsolete_year, by = c("Inception_Year" = "Obsolete_Year"),
                       suffix = c("_inception", "_obsolete")) %>% 
  rename(Year = Inception_Year) %>% 
  group_by(Year) %>% 
  mutate(Net_Count = sum(Count_inception, -Count_obsolete, na.rm = TRUE))
```

```{r data_overview_tbl, echo = FALSE}

datatable(Net_Count, colnames = c('Share_Classes_Launched' = 3, 'Share_Classes_Cut' = 4))

```

## By Month

```{r by_month_obsolete, echo =  FALSE}

Obsolete_Month <- function(x){
  Full %>% 
  filter(Obsolete_Year == x) %>% 
  mutate(Month = month(Obsolete..Date)) %>% 
  group_by(Month) %>% 
  count(name = paste("Obsolete_", x, sep = "") )
}


Years <- c("2018", "2019", "2020")

By_Month_Obsolete <- Years %>% 
  map(Obsolete_Month) %>% 
  reduce(full_join, by = "Month")

```

Share Class Liquidations & Mergers

```{r by_month_inception_graph, echo =  FALSE}

By_Month_Obsolete %>% 
  pivot_longer(
    !Month,
    names_to = "Year",
    values_to = "Count"
  ) %>% 
  mutate(Year = str_replace(Year, "Obsolete_", "")) %>% 
  ggplot(data = . , aes(x = Month, y = Count, color = Year)) +
  geom_line() +
  ggtitle("Liquidations By Month") +
  ylab("Count") +
  xlab("Month") +
  scale_x_continuous(breaks = seq(1, 12, by = 1)) +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))


```


```{r by_month_inception, echo =  FALSE}

Inception_Month <- function(x){
  Full %>% 
  filter(Inception_Year == x) %>% 
  mutate(Month = month(Inception..Date)) %>% 
  group_by(Month) %>% 
  count(name = paste("Inception_", x, sep = "") )
}


Years <- c("2018", "2019", "2020")

By_Month_Inception <- Years %>% 
  map(Inception_Month) %>% 
  reduce(full_join, by = "Month")

```

Share Class Launches by Month

```{r by_month_obsolete_graph, echo =  FALSE}

By_Month_Inception %>% 
  pivot_longer(
    !Month,
    names_to = "Year",
    values_to = "Count"
  ) %>% 
  mutate(Year = str_replace(Year, "Inception_", "")) %>% 
  ggplot(data = . , aes(x = Month, y = Count, color = Year)) +
  geom_line() +
  ggtitle("Fund Launches") +
  ylab("Count") +
  xlab("Month") +
  scale_x_continuous(breaks = seq(1, 12, by = 1)) +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))


```


## Liquidations

```{r obsolete_type, echo = FALSE}

ggplot(data = merge_type, mapping = aes(x =Obsolete_Year, y = Count, fill = Obsolete..Type)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_y_continuous(limits = c(0, 2500)) +
  coord_flip() +
  ggtitle("Fund Liquidations") +
  ylab("Count") +
  xlab("YEAR") +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))

merge_type %>% 
  filter(!is.na(Obsolete_Year)) %>% 
  pivot_wider(names_from = Obsolete..Type, values_from  = Count) %>% 
  datatable()

```


## New Funds

The data from Morningstar is at the share class level. This is a method to look at launches at the fund level.

```{r new_funds, message=FALSE}

#This looks at when a fund's oldest share class was created
#This assumes that the oldest share class's inception date is equal to the fund's creation date

New_Funds <- Full %>% 
  filter(Oldest..Share.Class == "Yes") %>% 
  group_by(Inception_Year, Index..Fund) %>% 
  summarise(Count = n(), .groups = "drop") %>% 
  mutate(
    Pct_Change = round(
      ((Count/lag(Count) - 1) * 100),
      1
    )) %>% 
  arrange(desc(Inception_Year))

```


```{r new_funds_graphs, echo=FALSE}

ggplot(data = New_Funds, mapping = aes(x =Inception_Year, y=Count, color=Index..Fund)) +
  geom_line() +
  ggtitle("New Fund Launches") +
  ylab("Count") +
  xlab("YEAR") +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))

New_Funds %>% 
  select(-Pct_Change) %>% 
  pivot_wider(names_from = Index..Fund, values_from = Count) %>%
  rename(
    Passive = Yes,
    Active = No
    ) %>% 
  group_by(Inception_Year) %>% 
  mutate(total = sum (Active, Passive, na.rm = TRUE)) %>% 
    datatable()

```

## New Funds (Alternative)

The method above uses the inception date of the _Oldest..Share.Class_ as a proxy for when a fund was created. This method uses _FundId_.

The oldest inception date related to a _FundID_ is a proxy for when the fund was created. Or the oldest obsolete date related to a a _FundID_ (with no active share classes) is likely when that fund was liquidated/merged.

There were some cases identified in the data where a FundID had no corresponding _Oldest..Share.Class_. This could be an error or signal that a fund's oldest share class is not captured in the data from Morningstar.

The results for 2020 ended up equal using the method above.

This method shows there were 6,224 funds in 2020 vs 6,076 in 2019 -- a change of 148 funds.
This method shows there were 6,076 funds in 2019 vs 5,876 in 2019 -- a change of 200 funds.


```{r fund_id, eval = FALSE, message = FALSE}

# a function to count share classes launched/liquidated before a certain date, grouped by their FundId
# the data is also grouped to show whether a Fund is Obsolete with all share classes having an obsolete date...
# or actie where at least one share class does NOT have an obsolete date in the data

status_count <- function(date_v){
  Full %>% 
  filter((Inception..Date < date_v | is.na(Inception..Date))
         )%>%
    mutate(Obsolete_Past = Obsolete..Date > date)
  group_by(FundId, Obsolete) %>% 
  summarise(Count = n(), .groups = "drop") %>%
  pivot_wider(names_from = Obsolete, values_from = Count, names_prefix = c("Count_")) %>% 
  mutate(
    Fund_Status = 
      case_when(
        is.na(Count_Exists) & Count_Obsolete >= 1 ~ "Obsolete_Fund",
        TRUE ~ "Exists_Fund"
      )
  )
}


#a breakdown of active vs. obsolete funds as of Dec. 31, 2020
status_2020 <- status_count("2021-01-01")

#a breakdown of active funds as of Dec. 31, 2020
status_summary_2020 <- status_2020 %>% 
  group_by(Fund_Status) %>% 
  count(name = paste("As_Of_"))

#a breakdown of active vs. obsolete funds as of Dec. 31, 2019
status_2019 <- status_count("2020-01-01")

#a breakdown of active vs. obsolete funds as of Dec. 31, 2019
status_summary_2019 <- status_2019 %>% 
  group_by(Fund_Status) %>% 
  count(name = paste("As_Of_"))

#a breakdown of active vs. obsolete funds as of Dec. 31, 2018
status_2018 <- status_count("2019-01-01")

#a breakdown of active vs. obsolete funds as of Dec. 31, 2018
active_2018 <- status_2018 %>% 
  filter(Fund_Status == "Active_Fund")


# obsolete_date <- Full %>% 
#   filter(FundId %in% obsolete$FundId) %>% 
#   group_by(FundId) %>% 
#   mutate(rank = rank(-as.numeric(Obsolete..Date), ties.method = "min")) %>% 
#   select(1:4, rank)
# 
# 
# inception_date <- Full %>% 
# filter(FundId %in% active_2020$FundId) %>% 
#   group_by(FundId) %>% 
#   mutate(rank = rank(as.numeric(Inception..Date), ties.method = "min")) %>% 
#   select(1:4, rank) %>% 
#   group_by(FundId, Inception..Date) %>% 
#   filter(rank == 1) %>% 
#   summarise()

```


# Companies

To look at which firms cut or added share classes I first clean up the company names with Branding.Names.Mod. 

```{r companies, message=FALSE}

company_count <- function(x, colz){
   Full %>% 
  mutate(
    Branding.Name.Mod =
    case_when(
      grepl("State Street", Branding.Name) ~ "State Street",
      grepl("TIAA", Branding.Name) ~ "TIAA/Nuveen",
      grepl("Nuveen", Branding.Name) ~ "TIAA/Nuveen",
      grepl("Eaton Vance", Branding.Name) ~ "Eaton Vance/Calvert",
      grepl("Calvert", Branding.Name) ~ "Eaton Vance/Calvert",
      grepl("iShares", Branding.Name) ~ "iShares/BlackRock",
      grepl("BlackRock", Branding.Name) ~ "iShares/BlackRock",
      grepl("PowerShares", Branding.Name) ~ "PowerShares/Invesco",
      grepl("Invesco", Branding.Name) ~ "PowerShares/Invesco",
      grepl("DWS$", Branding.Name) ~ "DWS/Xtrackers",
      grepl("Xtrackers", Branding.Name) ~ "DWS/Xtrackers",
      TRUE ~ Branding.Name)) %>% 
      group_by_at(colz) %>% 
      summarise(Count = n()) %>% 
    ungroup() %>% 
    arrange(desc(.[[2]]))
   }

comp_inception <- company_count(colz = c("Branding.Name.Mod", "Inception_Year"))
comp_obsolete <- company_count(colz = c("Branding.Name.Mod", "Obsolete_Year"))

comp_new_funds <- company_count(colz = c("Branding.Name.Mod", "Inception_Year", "Oldest..Share.Class")) %>% 
  filter(Oldest..Share.Class == "Yes")

comp_old_funds <- company_count(colz = c("Branding.Name.Mod", "Obsolete_Year", "Oldest..Share.Class")) %>% 
  filter(Oldest..Share.Class == "Yes")

comp_rank <- function(x){
  x %>%  filter(.[[2]] == "2020") %>% 
  mutate(Rank = rank(-Count)) %>% 
  filter(Rank <=20) %>%
    arrange(Rank)
}

top_cutters <- comp_rank(comp_obsolete)
top_old_cutters <-  comp_rank(comp_old_funds)

top_launchers <- comp_rank(comp_inception)
top_new_launchers <- comp_rank(comp_new_funds)

```

## Top Launchers

(Share Class)

```{r launchers, echo=FALSE}

datatable(top_launchers)

```

## Top New Fund Launchers

(Oldest Share Class)

```{r new_launchers, echo=FALSE}

datatable(top_new_launchers)

```


## Top Cutters

(Share Class)

```{r cutters, echo=FALSE}

datatable(top_cutters)

```

## Top Fund Cutters

(Oldest Share Class)

```{r old_cutters, echo=FALSE}

datatable(top_old_cutters)

```

# Share Class Overview

```{r share_class_overview, message=FALSE}

simp_count <- function(x, colz){
  Full %>%  
    group_by_at(colz) %>% 
    count(name = "Count")
  }

share_class_add_count <- simp_count(colz = c("Inception_Year", "Share.Class.Type"))

```

From Morningstar Direct

```{r share_class_overview_graphic, echo=FALSE}

ggplot(data = share_class_add_count, aes(x = Inception_Year, y = Count, fill = Share.Class.Type)) +
  geom_bar(stat = "identity") + 
  scale_y_continuous(breaks = seq.int(from = 1900, to = 2020, by = 20)) +
  facet_wrap(~Share.Class.Type, scales = "free_y") +
  ggtitle("New Fund Launches (Share Class Type") +
  ylab("Count") +
  xlab("YEAR") +
  theme_classic() +
  theme(plot.title = element_text(size = 11, face = "bold"))
```

## 2020

```{r share_class_2020, echo=FALSE}

share_class_add_count  %>% 
  filter(Inception_Year == "2020") %>% 
  arrange(desc(Count)) %>% 
  datatable()

```

## TK

```{r share_clas_view, eval = FALSE, echo=FALSE}

#The data from Morningstar is dirty. This pattern pulls the last letters/digits in the mutual fund name to identify the share class.

PATTERN <-
  c(
    "(\\s)?([:alnum:]+)?([:alnum:][:punct:][:alnum:] LW)?( Load Waived)?( LW)?([:punct:]+)?([:alnum:]+)?(-[:alnum:])?(™)?([:alpha:]\\{..\\} LW)?([:punct:]..[:punct:])?(\\s)?([:punct:][:alpha:]+[:punct:])?$"
  )

#The data from Morningstar is dirty. This pattern refines the pattern above to identify the share class.

CUT <-
  c(
    "([:punct:]\\X\\X[:punct:])|([:punct:]\\X\\X$)|(-)|(\\sLW)|(\\sLoad\\sWaived)|(\\([:alpha:]+\\))"
  )

#The data from Morningstar is dirty. This pattern makes manual changes to share classification for when the patterns above failed.

CHANGE <- c(
  "^Adm(in$)?(in[:alpha:]+)?$" = "Admin",
  "[:alnum:]+R6$" = "R6",
  "(?!AA)[:alpha:]+A$" = "A",
  "A\\{m" = "A",
  "([:alpha:]+)([:punct:])?([:alpha:]+)?A$" = "A",
  "(?![FIVS]+I$|([:alpha:]+)?II$)([:alpha:]+)?I$(?<!(IR$))" = "I",
  "([:alpha:]+)([:punct:])?([:alpha:]+)?I$(?<!(F/I$))" = "I",
  "([:alpha:]+)([:punct:])?([:alpha:]+)?C$" = "C",
  "PlusC([:digit:]+)$" = "C",
  "([:alpha:]+)?([:Punct:][:alpha:]+)?Inst(l$)?(itutional$)?$" = "Instl",
  "([:alnum:]+)?Ins$" = "Instl",
  "([:alnum:]+)?Insl$" = "Instl",
  "[:alnum:]+Ist$" = "Instl",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?Y$" = "Y",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?R$(?<!(SDR$)|(IR$))" = "R",
  "[:alnum:]+R5$" = "R5",
  "([:alpha:]+)?([:Punct:][:alpha:]+)?In([vest]+)?(ment$)?$(?<!(Int$))" = "Investor",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?N$" = "N",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?T$" = "T",
  "[:alnum:]+R4$" = "R4",
  "([:alpha:]+)?([:Punct:][:alpha:]+)?Adv(iser$)?(isory$)?$" = "Advisor",
  "([:alpha:]+)?([:Punct:][:alpha:]+)?ADV$" = "Advisor",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?K$" = "K",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?F$" = "F",
  "[:alnum:]+R3$" = "R3",
  "[:alnum:]+F3$" = "F3",
  "([:alpha:]+)?([:punct:])?([:alpha:]+)?IS$" = "IS",
  "^I$" = "Instl",
  "Retire(ment)?$" = "R"
)


#Using the above patterns we can clean the share class information
#Name -> Extract -> New -> Revision

INC_Data <- Full %>%
  select(Name) %>%
  mutate(
    Match = str_detect(Name, PATTERN),
    EXTRACT = str_extract(Name, PATTERN),
    New = str_trim(str_replace_all(EXTRACT,  CUT, "")),
    Revision = str_replace_all(New, CHANGE)
    )

#This shows how the names were altered to create the final share class informatin (Revision)
#The revision counts might be inaccurate.
INC_Changes <- INC_Data %>%
  group_by(EXTRACT) %>%
  summarise(cases = n()) %>%
  mutate(New = str_trim(str_replace_all(EXTRACT,  CUT, ""))) %>% group_by(New) %>%
  mutate(replacements = n() - 1,
         newcases = sum(cases)) %>%
  mutate(Revision = str_replace_all(New, CHANGE)) %>%
  group_by(Revision) %>%
  mutate(replacements1 = (n() - replacements) - 1,
         newercases = sum(cases))

#This counts the number of share class types in the data
INC_Summary <- INC_Data %>%
  group_by(Revision) %>%
  summarise(cases = n()) %>%
  arrange(-cases)

INC_MODIFIED <- INCEPTION %>%
  mutate(
    Match = str_detect(Name, PATTERN),
    EXTRACT = str_extract(Name, PATTERN),
    New = str_trim(str_replace_all(EXTRACT,  CUT, "")),
    Revision = str_replace_all(New, CHANGE),
    `Obsolete Year` = year(`Obsolete Date`),
    `Obsolete Month` = month(`Obsolete Date`),
    `Inception Year` = year(`Inception Date...4`),
    `Inception Month` = month(`Inception Date...4`)
  ) %>%
  select(Name, 
         Match, 
         EXTRACT, 
         New, 
         Revision, 
         2:3,
         `Obsolete Month`,
         `Obsolete Year`, 
         4, 
         `Inception Month`, 
         `Inception Year`, 
         5:22)

```